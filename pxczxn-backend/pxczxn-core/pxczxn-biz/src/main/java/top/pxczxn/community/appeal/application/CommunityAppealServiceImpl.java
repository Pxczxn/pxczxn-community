...
public class CommunityAppealServiceImpl implements CommunityAppealService {
    ...
    @Override @Transactional public CommunityAppealView submit(Long appellant, Long reportId, String reason, String evidenceJson) {
        CommunityReport report = report(reportId); if (!"RESOLVED".equals(report.getStatus())) throw new BusinessException(409, "Only resolved reports can be appealed");
        if (!owns(appellant, report)) throw new BusinessException(403, "Only the reported target owner can appeal");
        ...
        try { if (appealMapper.insert(appeal) != 1) throw new BusinessException(500, "Unable to submit appeal"); } catch (DuplicateKeyException exception) { throw new BusinessException(409, "An appeal already exists for this report"); }
    ...
    @Override @Transactional(readOnly = true) public AppealContextView context(Long appellant, Long reportId) { CommunityReport report = report(reportId); if (!"RESOLVED".equals(report.getStatus()) || !owns(appellant, report)) throw new BusinessException(404, "Appealable report does not exist"); return new AppealContextView(report.getId(), report.getTargetType(), report.getTargetId(), report.getResolutionCode(), report.getResolutionNote(), report.getLockVersion()); }
    ...
    private CommunityAppeal appeal(Long id) { CommunityAppeal value = appealMapper.selectById(positive(id, "appeal")); if (value == null) throw new BusinessException(404, "Appeal does not exist"); return value; }
    ...
    private static String required(String value, int max, String label) { if (value == null || value.trim().isEmpty()) throw new BusinessException(400, "Missing " + label); String result = value.trim(); if (result.length() > max) throw new BusinessException(400, "Value is too long"); return result; }
    ...
    private static BusinessException conflict() { return new BusinessException(409, "Appeal state has changed; refresh and retry"); }
}
