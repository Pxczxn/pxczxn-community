...
public class ContentRuleServiceImpl implements ContentRuleService {
    ...
    public ContentRuleView create(Long adminId, RuleCommand command) {
        if (adminId == null || adminId <= 0) throw new BusinessException(401, "Administrator login required");
        ...
        try { if (mapper.insert(rule) != 1) throw new BusinessException(500, "Unable to create content rule"); } catch (DuplicateKeyException exception) { throw new BusinessException(409, "An active rule for this keyword already exists"); }
    ...
    public ContentRuleView update(Long ruleId, RuleCommand command) {
        ...
        try { mapper.update(null, update); } catch (DuplicateKeyException exception) { throw new BusinessException(409, "An active rule for this keyword already exists"); }
    ...
    private ContentKeywordRule require(Long id) { if (id == null || id <= 0) throw new BusinessException(400, "Invalid rule id"); ContentKeywordRule rule = mapper.selectById(id); if (rule == null || rule.getDeletedAt() != null) throw new BusinessException(404, "Content rule not found"); return rule; }
    private static RuleFields fields(RuleCommand command, boolean partial) {
        if (command == null) throw new BusinessException(400, "Rule payload is required");
        String keyword = command.keyword() == null ? null : text(command.keyword(), 200, "keyword");
        if (!partial && keyword == null) throw new BusinessException(400, "keyword is required");
        ...
        Integer sort = command.sortOrder(); if (sort != null && (sort < 0 || sort > 100000)) throw new BusinessException(400, "Invalid sort order");
    ...
    private static String scopes(String raw) { String value = raw.trim().toUpperCase(Locale.ROOT); if (value.equals("ALL")) return "ARTICLE,COMMENT,MOMENT"; LinkedHashSet<String> result = new LinkedHashSet<>(); Arrays.stream(value.split(",")).map(String::trim).filter(s -> !s.isEmpty()).forEach(scope -> { if (!SCOPES.contains(scope)) throw new BusinessException(400, "Invalid content scope"); result.add(scope); }); if (result.isEmpty()) throw new BusinessException(400, "At least one content scope is required"); return String.join(",", result); }
    private static String enumValue(String raw, Set<String> allowed, String label) { String value = raw.trim().toUpperCase(Locale.ROOT); if (!allowed.contains(value)) throw new BusinessException(400, "Invalid " + label); return value; }
    private static String text(String raw, int max, String label) { String value = Normalizer.normalize(raw.trim(), Normalizer.Form.NFKC); if (value.isBlank() || value.length() > max) throw new BusinessException(400, "Invalid " + label); return value; }
    private static String optional(String raw, int max, String label) { String value = Normalizer.normalize(raw.trim(), Normalizer.Form.NFKC); if (value.length() > max) throw new BusinessException(400, "Invalid " + label); return value.isBlank() ? null : value; }
