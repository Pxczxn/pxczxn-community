...
export const useSiteStore = defineStore('site', () => {
  // 站点名称
  const siteName = ref('星语社区运营中心')
  // 站点描述
  const siteDescription = ref('博客社区运营与治理中心')
  // 站点 Logo
  const siteLogo = ref('')
  // 版权信息
  const copyright = ref('')
  ...
  async function loadConfig() {
    try {
      ...
      if (config.system) {
        siteName.value = config.system.siteName || '星语社区运营中心'
        siteDescription.value = config.system.siteDescription || '博客社区运营与治理中心'
