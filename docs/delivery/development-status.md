...
| M4 平台治理与内容安全 | 完成 | 举报、屏蔽、申诉、处罚、内容规则、反滥用与治理看板均已交付并完成验收记录 |
| M5 发现、搜索与创作者体验 | 完成 | 搜索、可解释发现、SEO、RSS、统计、性能与完整验收均已交付 |
| M6 公网发布与持续运营 | 开发中 | 原生 systemd/Nginx/私有 MinIO 已上线，旧博客服务已退役；备份恢复、一致回滚与飞书告警送达演练已通过。按用户选择不启用 GitHub 自动发布，仍待真实灰度运营验收。 |
