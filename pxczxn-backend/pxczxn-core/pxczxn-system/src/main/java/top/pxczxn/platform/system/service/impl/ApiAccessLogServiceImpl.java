...
import top.pxczxn.platform.system.entity.ApiAccessLog;
import top.pxczxn.platform.system.mapper.ApiAccessLogMapper;
import top.pxczxn.platform.system.service.ApiAccessLogService;
    ...
    public void pushToRedis(ApiAccessLog accessLog) {
        if (redisAvailable) {
            try {
                ...
                log.warn("Redis 不可用，API 访问日志改为直接写入 MySQL");
            }
        }
        try {
            save(accessLog);
        } catch (Exception e) {
            log.error("API 访问日志写入 MySQL 失败", e);
        }
    }
