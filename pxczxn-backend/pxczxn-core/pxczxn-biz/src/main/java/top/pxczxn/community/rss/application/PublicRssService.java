...
public class PublicRssService {
    ...
    private final PlatformTagMapper tagMapper; private final ArticleTagMapper articleTagMapper; private final SeriesMapper seriesMapper; private final SeriesArticleMapper chapterMapper;
    @Transactional(readOnly=true) public RssDocument site(String origin){return document("星语社区 - 最新文章", "全站公开文章", origin+"/articles", articleService.discover(new PublicDiscoveryQuery(null, null, null, 1, 30)).records());}
    ...
    @Transactional(readOnly=true) public RssDocument series(String origin,Long seriesId){Series series=seriesMapper.findPublicById(seriesId);if(series==null)throw new BusinessException(404,"系列不存在");List<PublicArticleSummaryView> records=new ArrayList<>();for(SeriesArticle chapter:chapterMapper.findBySeries(seriesId)){try{var d=articleService.detail(chapter.getArticleId());records.add(summary(d));}catch(BusinessException ignored){}}return document(series.getTitle()+" - RSS",Optional.ofNullable(series.getSummary()).orElse("系列公开章节订阅"),origin+"/series/"+seriesId,records);}
