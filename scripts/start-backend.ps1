...
# Workaround: PowerShell 5.1 throws on Start-Process when both 'Path' and 'PATH'
# environment keys coexist in the process block. Collapse them into a single
# canonical 'Path' key so the child process environment can be built.
try {
    $pathVal = [Environment]::GetEnvironmentVariable('PATH', [System.EnvironmentVariableTarget]::Process)
    if ($null -eq $pathVal) { $pathVal = [Environment]::GetEnvironmentVariable('Path', [System.EnvironmentVariableTarget]::Process) }
    if ($null -ne $pathVal) {
        [Environment]::SetEnvironmentVariable('PATH', $null, [System.EnvironmentVariableTarget]::Process)
        [Environment]::SetEnvironmentVariable('Path', $null, [System.EnvironmentVariableTarget]::Process)
        [Environment]::SetEnvironmentVariable('Path', $pathVal, [System.EnvironmentVariableTarget]::Process)
    }
} catch { }
...
$process = Start-Process -FilePath 'java.exe' -ArgumentList '-jar', $jar, '--server.port=8849', '--spring.profiles.active=local' -WorkingDirectory $backendRoot -RedirectStandardOutput $logFile -RedirectStandardError "$logFile.err" -PassThru -WindowStyle Hidden
