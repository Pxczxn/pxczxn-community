...
public interface FollowingFeedMapper {
    @Select("""
            SELECT * FROM (
              ...
              FROM community_follow f JOIN blog b ON b.id=f.target_id AND b.status='ACTIVE' AND b.deleted_at IS NULL
              JOIN team t ON t.blog_id=b.id AND t.status='ACTIVE' AND t.deleted_at IS NULL
              JOIN team_series s ON s.team_id=t.id AND s.deleted_at IS NULL AND s.review_status='APPROVED'
    ...
    @Select("""
            SELECT COUNT(*) FROM (
              ...
              UNION ALL SELECT s.id FROM community_follow f JOIN blog b ON b.id=f.target_id AND b.status='ACTIVE' AND b.deleted_at IS NULL JOIN team t ON t.blog_id=b.id AND t.status='ACTIVE' AND t.deleted_at IS NULL JOIN team_series s ON s.team_id=t.id AND s.deleted_at IS NULL AND s.review_status='APPROVED' WHERE f.follower_user_id=#{userId} AND f.target_type='BLOG'
