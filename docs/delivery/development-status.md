...
下一阶段为生产硬化，包括公网部署环境、独立数据库、CI 持续 CVE/SBOM 扫描、
管理端大块分包和脚手架遗留命名清理。
...
- 当时本地验证暂复用已有后台数据库；阶段 5 已安全迁移到
  `pxczxn_community`，独立环境继续使用
  `pxczxn_community` 数据库。
- 管理端原脚手架仍有一批历史严格 TypeScript/noUnused 报错；社区新增页面
  已完成自身类型收口，Vite 生产构建正常。
