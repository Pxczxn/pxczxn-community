...
export default function ReportsPage() {
  ...
  const load = () => { setLoading(true); void communityApi.myReports().then(setItems).catch((error) => setMessage(error instanceof Error ? error.message : "加载失败")).finally(() => setLoading(false)); };
  useEffect(load, []);
  async function submit(event: FormEvent) { event.preventDefault(); setMessage(""); try { await communityApi.createReport({ targetType, targetId, reasonCode, description }); setTargetId(""); setReasonCode(""); setDescription(""); load(); } catch (error) { setMessage(error instanceof Error ? error.message : "提交失败"); } }
