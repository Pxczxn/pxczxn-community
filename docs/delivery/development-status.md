...
| M2.5 产品语义与信息架构校正 | 待开发 | 作为下一阶段，先完成发现页、入口、动态路由、管理端社区化和文案清理 |
...
星语社区当前基线为 V1（M1 + M2）加即时聊天模块，状态：完成。

下一阶段调整为 **M2.5 产品语义与信息架构校正**。它不是新增团队、治理或增长大业务，而是把已有真实内容、互动、通知和聊天能力放到正确的平台入口与导航中：首次进入博客端应是社区发现页，进入管理端应是社区运营中心。

M2.5 完成后，按 M3 → M4 → M5 → M6 顺序继续推进；阶段不得并行混入同一提交。

## 当前工程债务与后续处理

- M2.5 前台仍保留若干原型硬编码团队、动态和协作文章路由，必须改为真实参数化路由并清除运行时示例数据依赖。
- 管理端部分菜单和文案仍体现通用管理脚手架语义；M2.5 负责将其收敛到社区运营中心，同时保留系统设置中的平台人员与权限。
