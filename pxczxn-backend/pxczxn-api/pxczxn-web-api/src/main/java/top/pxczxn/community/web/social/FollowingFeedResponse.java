...
public record FollowingFeedResponse(List<Item> records, long total, int pageNum, int pageSize) {
    ...
    public record Item(String itemType, String targetId, String title, String excerpt, String canonicalPath, String authorName, String blogName, String tagName, LocalDateTime occurredAt) {
        static Item from(FollowingFeedItemView view) { return new Item(view.itemType(), view.targetId().toString(), view.title(), view.excerpt(), view.canonicalPath(), view.authorName(), view.blogName(), view.tagName(), view.occurredAt()); }
    }
}
