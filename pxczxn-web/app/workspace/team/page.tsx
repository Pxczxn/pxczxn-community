    ...
    {missingTeam ? <p role="alert" style={{ color: "var(--danger)" }}>请选择一个团队工作台。</p>
      : error ? <p role="alert" style={{ color: "var(--danger)" }}>{error}</p>
        : !workspace ? <div className="text-center" style={{ padding: 80 }}><Loader2 className="animate-spin" /></div>
          : <><section className="surface" style={{ padding: 24, marginBottom: 18 }}><h1 style={{ margin: 0 }}>{workspace.team.team.name}</h1><p className="secondary">M3 团队协作创作 · 你的团队角色：{workspace.viewerRole}</p></section><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>{workspace.capabilities.map((capability) => capability === "SERIES" ? <Link className="surface" key={capability} href={`/workspace/team/series?teamId=${encodeURIComponent(teamId)}`} style={{ padding: 18, color: "inherit", textDecoration: "none" }}><strong>{label(capability)}</strong><p className="secondary" style={{ marginBottom: 0 }}>创建系列、编排章节并提交平台审核。</p></Link> : <section className="surface" key={capability} style={{ padding: 18 }}><strong>{label(capability)}</strong><p className="secondary" style={{ marginBottom: 0 }}>此能力由服务端角色权限授予。</p></section>)}</div></>}
  </main></>;
}

function label(value: string) { return ({ OVERVIEW: "概览", ARTICLES: "文章管理", MEMBERS: "成员管理", CATEGORIES: "分类", SERIES: "系列管理", SETTINGS: "团队设置" }[value] || value); }
