...
import top.pxczxn.platform.system.mapper.SysConfigGroupMapper;
...
public class SysConfigGroupServiceImpl implements SysConfigGroupService {
    ...
    public void saveConfig(String groupCode, String configValue) {
        ...
        try {
            ...
            log.warn("Redis 不可用，系统配置仅更新本地缓存: groupCode={}", groupCode);
        }
    }
            ...
            for (SysConfigGroup config : list) {
                if (config.getConfigValue() != null) {
                    redisTemplate.opsForValue().set(
                            CACHE_KEY_PREFIX + config.getGroupCode(),
                            config.getConfigValue()
                    );
                }
            }
        } catch (RuntimeException e) {
            log.warn("Redis 不可用，系统配置使用进程内缓存");
        }
