<template>
  <div class="community-page">
    <header class="page-heading">
      <div><div class="page-eyebrow">SERIES REVIEW</div><h1>系列审核</h1><p>审核团队创建的文章系列；通过后才会在社区公开展示。</p></div>
...
const columns = [{ title: '系列', key: 'title', minWidth: 220, render: (row: CommunitySeries) => h('div', [h('strong', row.title), h('small', { class: 'muted' }, ` ${row.slug}`)]) }, { title: '团队 ID', key: 'teamId', width: 140 }, { title: '章节', key: 'chapters', width: 100, render: (row: CommunitySeries) => `${row.chapters.length} 篇` }, { title: '连载状态', key: 'serializationStatus', width: 120, render: (row: CommunitySeries) => h(NTag, { size: 'small', type: row.serializationStatus === 'COMPLETED' ? 'success' : row.serializationStatus === 'PAUSED' ? 'warning' : 'info' }, { default: () => statusLabel(row.serializationStatus) }) }, { title: '提交时间', key: 'updatedAt', width: 175, render: (row: CommunitySeries) => formatDateTime(row.updatedAt) }, { title: '操作', key: 'actions', width: 180, fixed: 'right' as const, render: (row: CommunitySeries) => h(NSpace, { size: 'small' }, { default: () => [h(NButton, { size: 'small', type: 'success', onClick: () => open(row, 'approve') }, { default: () => '通过' }), h(NButton, { size: 'small', type: 'error', onClick: () => open(row, 'reject') }, { default: () => '驳回' })] }) }]
