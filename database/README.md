...
- 在线校验：V001 至 V038 各有一个 `database/verify` 脚本
...
关闭并脱敏支付、短信、微信和第三方集成；V016 记录原脚手架清理决策；V017
按修订后的保留策略恢复可扩展菜单元数据，其中未启用能力继续保持隐藏和停用；
V018 恢复无密钥、默认关闭的集成配置占位；V019 增加聊天单方清除状态、群聊
已读游标、查询索引和状态约束；V020-V022 完成 M2.5 信息架构与保留脚手架能力；
V023-V029 完成 M3 团队、投稿、协作、聊天和系列；V030-V035 完成 M4 治理与反滥用；
V036-V038 完成 M5 搜索索引、运营合集和创作者分析。
...
| `checksum` | SQL 文件按 UTF-8、LF 换行规范化后的 SHA-256；避免 Windows CRLF 与 CI/Linux LF 造成伪漂移 |
